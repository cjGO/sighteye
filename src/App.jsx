...
const PortraitContent = () => {
  // --- Refs (Must stay in the view component) ---
  ...
  const getImageStyle = (isRight) => {
      ...
      const tx = -viewTransform.x * viewTransform.scale;
  ...
  const { handlePointerDown, handlePointerMove, handlePointerUp } = useCanvasInput({
    ...ctx, // Pass entire context to the hook
    containerRef, 
    ...
    const renderLayers = (targetCtx) => {
        layers.forEach(layer => {
            ...
            if (layer.id === activeLayerId && mode === 'draw' && currentPoints.length > 0) {
                if (tempCtx) drawVariableWidthPath(tempCtx, currentPoints, brushSize, brushColor, brushOpacity, isEraser, usePressure);
            }
...
const handleSave = () => {
    ...
    img.onload = () => {
        ...
        if (imgRatio > paneRatio) { drawW = targetW; drawH = targetW / imgRatio; offsetX = targetX; offsetY = targetY + (targetH - drawH) / 2; } 
